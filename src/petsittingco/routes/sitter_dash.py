...
def jobs():
    ...
    for job in job_array:
        if not job.canceled and not job.accepted:
            jobs += '<div class="row" style="border:1px solid black"><h3 style="text-align: center">Job for ' + job.owner.first_name + ' on ' + job.start_datetime +  '</h3><br></p><button onclick="document.location=\'../petsitterdashboard/accept.html?job_id=' + job.id + '\'"  id="submit-button" class="custom-btn btn-bg btn mt-3 form-control" data-aos-delay="300" >Accept Job</button> </div><br>'
