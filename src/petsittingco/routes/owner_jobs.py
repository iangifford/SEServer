...
def jobs():
    ...
    for job in job_array:
        if not job.canceled and job.owner == current_user:
            ...
            jobs += '<div class="row"><h3 style="text-align: center">' + sitter_name + ' on ' + job.start_datetime +  '</h3></div><div class="row"><button onclick="document.location=\'../petownerdashboard/job.html?job_id=' + job.id + '\'"  id="submit-button" class="custom-btn btn-bg btn mt-3" data-aos-delay="300" >View Job</button> </div><br>'
